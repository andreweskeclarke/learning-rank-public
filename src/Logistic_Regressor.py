...
def lr_sgd_optimiser(n_iterations, n_labels,X,labels,alpha,queries):
    ...
    theta = np.random.rand(len(set(labels)),X.shape[1]) * 1
...
def return_probs(thetas,X):
    ...
    X = np.c_[np.ones(X.shape[0]), X]
    probs = np.ones((thetas.shape[0],X.shape[0]))
    for i in range(thetas.shape[0]):
        probs[i,:] = phi((np.multiply(thetas[i,:],X)).sum(axis=1))
...
def save_thetas(n_iterations,a):
    ...
    alpha=a,
    queries=query_ids)
    THETAS = 'thetas_' + str(n_iterations) + '_' + str(a)
...
def load_thetas():
    thetas = np.load('../data/thetas_1000000_1e-05.npy')
    return thetas
...
probs_sums = np.divide(np.sum(np.multiply(probs_valid,[0,1,2,3,4]),axis=1),np.sum(probs_valid,axis=1))
...
probs_sums_test = np.divide(np.sum(np.multiply(probs_test,[0,1,2,3,4]),axis=1),np.sum(probs_test,axis=1))
